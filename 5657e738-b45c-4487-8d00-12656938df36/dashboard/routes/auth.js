...
const Auth = Router()
	.get("/login", async (req, res) => {
		if (req.query.code) {
			...
			params.set(
				"redirect_uri",
				`${req.dashboardConfig.baseUrl}${
					req.dashboardConfig.noPortIncallbackUrl
						? ""
						: ":" + req.dashboardConfig.port
				}/auth/login`
			);
			...
			res.redirect(
				`https://discordapp.com/api/oauth2/authorize?client_id=${req.client?.user?.id}&scope=identify%20email%20guilds&response_type=code`
			);
		}
	})
