...
class Music(commands.Cog):
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
       ...
       Если канал не указан, он присоединяется к вашему каналу.
        """
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            return await ctx.send('Я и так не подключен. Зачем меня кикать?')
    ...
    async def _pause(self, ctx: commands.Context):
        """Приостанавливает проигрываемую в данный момент песню."""
...
bot = commands.Bot('-', description='Крутой бот от меня.')
...
async def on_ready():
    ...
    await bot.change_presence(activity=discord.Game(name="пиши что хочешь"))
    print("Bot is ready!")

bot.run('ODU5MTEzNDc0MDQxNTc3NTMy.YNn9xQ.rZJ28_W7GtYI2EiXVNC45l0_Lfs')
