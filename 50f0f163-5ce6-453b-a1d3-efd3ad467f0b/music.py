...
class Player(commands.Cog):
    ...
    async def volume(self,ctx,volume:int=None):
      if volume is None:
        await ctx.send("bruh<:bruh:887896562972389446> provide me the volume bro, 1 to 10 btw")

      if volume > 10:
        await ctx.send("your volume is way too thicc")
      else:

        ctx.voice_client.source.volume=volume/10
        await ctx.send(f"The volume has being set to {volume}")
