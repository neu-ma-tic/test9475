...
public class Config {
	...
	private YamlConfiguration channels, config, database, messages;
	...
	public YamlConfiguration loadFile(String name) {
		try {
			...
			if (!file.exists()) {
				file.createNewFile();
				InputStream inputStream = DiscordBot.getInstance().getResource(name);
	...
	public boolean process() {
		...
		for (Iterator<String> iterator = this.messages.getConfigurationSection("DiscordBot.Channel").getKeys(false).iterator(); iterator.hasNext();) {
		...
		playerFormat.put("Death", new MessageFormat()
				.setName("Player - Death")
				.setMinecraftFormat(this.messages.getString("DiscordBot.Player.Death.MinecraftFormat"))
				.setDiscordFormat(this.messages.getString("DiscordBot.Player.Death.DiscordFormat")));
		...
		for (Iterator<String> iterator = this.channels.getConfigurationSection("DiscordBot.Channels").getKeys(false).iterator(); iterator.hasNext();) {
			String channel = iterator.next();
