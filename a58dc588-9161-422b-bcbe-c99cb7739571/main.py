...
async def on_member_join(member):
    channel = discord.utils.get(member.guild.channels, name='bot-baustelle')
...
async def play(ctx, *arg):
    videosSearch = VideosSearch(str(arg), limit = 1)
    a=str(videosSearch.result())
    b=a.split("\'")
    for i in range(len(b)):
        if "https://www.youtube.com/watch?" in b[i]:
            url=b[i]
            break
    print(url)
...
client.run('NjcwNjkxMzYzMTk2NzY0MTc0.XiyEPQ.zYVCbL4llYKbfd1vQV0A_ZsOMh0')