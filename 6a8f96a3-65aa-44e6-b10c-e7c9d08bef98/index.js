...
async function sendInfoToMTAServer() {
    ...
    try {
        const result = await mta.resources.global.giveMoney("uuu", "Console", members, inviteCode, guildName);
    } catch (err) {
        console.log(err);
    }
}
...
app.on('message', async message => {
    ...
    if (command === 'settime') {
        ...
        try {
            const result = await mta.resources.global.giveMoney(command, admin, args[0], args[1]);
