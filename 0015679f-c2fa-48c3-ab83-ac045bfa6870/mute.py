            ...
            if len(command) >= 1:
                if is_integer(command[0]):
                    ...
                    if len(command) >= 2:
                        ...
                        temp = [
                            item for item in command if command.index(item) > 0
                        ]
                        reason = " ".join(temp)
                    else:
                    ...
                    if user is not None:
                        ...
                        await user.add_roles(
                            ...
                            reason=f"Muted by {message.author.name}")
                        self.storage.settings["guilds"][guild_id][
                            "muted_users"][str(user_id)] = {}
                        self.storage.settings["guilds"][guild_id][
                            "muted_users"][str(user_id)]["duration"] = -1
                        self.storage.settings["guilds"][guild_id][
                            "muted_users"][str(user_id)]["reason"] = reason
                        self.storage.settings["guilds"][guild_id][
                            "muted_users"][str(
                                user_id)]["normal_duration"] = -1
                        await self.storage.write_settings_file_to_disk()
                        # Message the channel
                        await message.channel.send(
                        ...
                        await embed_builder.add_field(
                            name="**Executor**",
                            value=f"`{message.author.name}`")
                        await embed_builder.add_field(name="**Muted user**",
                                                      value=f"`{user.name}`")
                ...
                if is_integer(command[0]):
                    ...
                    if is_valid_duration(duration):
                        ...
                        if user is not None:
                            ...
                            self.storage.settings["guilds"][guild_id][
                                "muted_users"][str(
                                    user_id)]["duration"] = mute_duration
                            self.storage.settings["guilds"][guild_id][
                                "muted_users"][str(user_id)]["reason"] = reason
                            self.storage.settings["guilds"][guild_id][
                                "muted_users"][str(
                            ...
                            # Message the channel
                            await message.channel.send(
                                f"**Temporarily muted user:** `{user.name}` **for:** `{command[1]}`**. Reason:** `{reason}`"
                            )
                        ...
                        await message.channel.send(
                            self.invalid_duration.format(user_id=user_id,
                                                         usage=self.usage))
                else:
                    await message.channel.send(
                        self.not_a_user_id.format(user_id=command[0],
