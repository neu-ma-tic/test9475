...
# #RPS_________________________________________________________________________________________________________
# # event______________________________________________________________________________________________________________________
...
#   if message.content.startswith("!playgg"): # the playGG command to start the guessing game.
#       channelG = client.get_channel(830660169208692736)
#       if message.channel != channelG:
#         channelT = message.channel
#         await channelT.send("Go to *Guessing Game* channel to play")
#       else:
#         await channelG.send("Choose a number between 1-10. Enter numerical values only.") #message that tells about the start of the game
...
#           users[message.author.name] = 110
#         else:
#           users[message.author.name] += 10
#         await channelG.send(f'Correct answer {message.author.name}, you were awarded 10 gold'.format(msg))

# # commands______________________________________________________________________________________________________________________
# @client.command()
...
#   embed.add_field(name = "!playgg :", value = "Play a guessing game to earn gold", inline = False)
...
async def on_message(message):
    if message.content.startswith("!playgg"): # the playGG command to start the guessing game.
        channelG = client.get_channel(830660169208692736)
        if message.channel.id != 830660169208692736:
          channelT = message.channel
          await channelT.send("Go to *Guessing Game* channel to play")
        else:
          await channelG.send("Choose a number between 1-10. Enter numerical values only.") #message that tells about the start of the game
