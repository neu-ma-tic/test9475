import random
from modules.Chess.AI import pieces, ai
...
class Board:
    ...
    def perform_move(self, move):
        ...
        if (move.castling_move):
            if (move.xto < move.xfrom):
                rook = self.chesspieces[0][Board.WIDTH-1]
                rook.x = 3
                self.chesspieces[3][Board.WIDTH-1] = rook
                self.chesspieces[0][Board.WIDTH-1] = 0
            if (move.xto > move.xfrom):
                rook = self.chesspieces[Board.HEIGHT-1][Board.WIDTH-1]
                rook.x = Board.HEIGHT-3
                self.chesspieces[Board.WIDTH-3][Board.HEIGHT-1] = rook
                self.chesspieces[Board.HEIGHT-1][Board.WIDTH-1] = 0
    ...
    def __repr__(self):
        ...
        emo = ('😃', '😏', '😁', '😆', '😅', '🤣', '😂', '🙂', '🙃', '😉', '😊', '🤨', '😐', '😶', '😒', '🙄', '😬', '😎')
        characters = (
            "<:A_:885129794554048513>"
            "<:B_:885129794604396564>"
            "<:C_:885129794767949854>"
            "<:D_:885129794302390344>"
            "<:E_:885129794856030208>"
            "<:F_:885129794570842132>"
            "<:G_:885129794134614048>"
            "<:H_:885129794461765682>"
        )

        string =  f"{random.choice(emo)}{''.join(characters)}\n"
        for y in range(Board.HEIGHT):
            string += digits[7 - y]
            for x in range(Board.WIDTH):
                piece = self.chesspieces[x][y]
