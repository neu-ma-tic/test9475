...
module.exports = {
    ...
    async execute(client, message, args, Discord, cmd) {
        ...
        if(!voice_channel) return message.channel.send('You need to be in a voice channel to execute that command!');
        ...
        if(cmd === 'play') {
            ...
            if(!server_queue) {
                ...
                queue.set(message.guild.id, queue_cunstructor);
                queue_cunstructor.songs.push(song);
    
                try {
                    const connection = await voice_channel.join();
                    queue_cunstructor.connection = connection;
                    video_player(message.guild, queue_cunstructor.songs[0]);
                } catch (err) {
